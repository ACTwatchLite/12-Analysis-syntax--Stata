...
*SAVE FULL DATA SET
	*Compress and save data set
	...
	save "${mngmtdata}/${country}_${year}_full_data.dta", replace
	*for use in results generation 
	
	save "${finaldata}/${country}_${year}_full_data.dta", replace 
	*for final dataset storage; future analysis
	
/* MERGE IN SUPPLIER DATASET

	use "${cleandir}/AwL_${country}_${year}-suppliers_clean.dta", clear
	
	drop key
	rename parent_key key
	duplicates report key
	
	merge m:m key using "${mngmtdata}/${country}_${year}_full_data.dta"
...
*SAVE OUTLET-LEVEL DATA SET
	*Reopen full data set
	...
	use "${mngmtdata}/${country}_${year}_full_data.dta", clear
	...
	*Compress and save data set
	save "${mngmtdata}/${country}_${year}_outlet_data.dta", replace
	*for use in results generation 
	
	save "${finaldata}/${country}_${year}_outlet_data.dta", replace 
	*for final dataset storage; future analysis
...
*SAVE RDT DATA SET
	...
	*Compress and save data set
	...
	save "${mngmtdata}/${country}_${year}_rdt_data.dta", replace
	*for use in results generation 
	
	save "${finaldata}/${country}_${year}_rdt_data.dta", replace 
	*for final dataset storage; future analysis
...
*SAVE AM DATA SET
	...
	*Keep only RDT and micro products
	keep if productype==1 | productype==2
	recode qardt (.=0) 
	...
	*Compress and save data set
	...
	save "${finaldata}/${country}_${year}_antimalarial_data.dta", replace 
	*for final dataset storage; future analysis
