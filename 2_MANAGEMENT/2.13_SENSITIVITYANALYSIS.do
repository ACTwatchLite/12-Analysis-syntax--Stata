...
*Pharmacies
list gname a3 volume if volume>500 & volume!=. & (pop==1 | drs==1) & booster==0 
/* ADD RESULTS HERE: 

*/
replace volume=. if volume>500 & volume!=. & (pop==1 | drs==1) & booster==0
...
/* !!!All volumes with outliers >100 AETD should be investigated regardless of outlet type. Mark volumes as 'plausible' if the volume, although unlikely, could have been
	 ...
	 ** drsvs
list gname a3 volume if volume>100 & volume!=. & drs==1 & booster==0
/* ADD RESULTS HERE: 
...
*Pharmacies
list outcat2 brand manu volume  if volume>500 & volume!=. & (pop==1 | drs==1) & booster==0
/*!!!Copy and paste the stata output into the do file
