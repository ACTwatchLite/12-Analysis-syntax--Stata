...
Use the weighting tool template provided in the toolkit (file pathway below) to organize cluster/ study areas, population, and strata -- the tool will provide weights 

Each cluster/ study area should have been assigned to whatever strata is being used by the study e.g. urban/ rural using existing data sources such as census, other government or offical datasets, or other sources as needed. Document sources and add this information to results output.


import excel "...12 Analysis syntax\2_MANAGEMENT\AwL Weighting Tool.xlsx"", sheet("Main cluster") firstrow clear
drop D E 
rename type area
		...
		*for TST dataset
			gen booster=0
				...
				*!!!The following dummy variables are generated specifically for the detaile sample description table (Table X2).
					gen boosterT=1
					gen bootsterN=1 if booster1==0
					gen bootsterY=1 if booster1==1
...
gen booster_wt=.75 if c2==11 & (c3==106 | c3==111) & c7==11

		gen wt_marketShare=main_wt1
...
gen wt_allOutlet=booster_wt if c2==11
...
*Generate FPC variable (Finite population correction) 
	...
	*/
	*note these are random for TST data
	
	replace fpc=(65)/(124) if strata==11 // Stratum 1 name 
	replace fpc=(129)/(299) if strata==22 // Stratum 2 name 
	replace fpc=(63)/(610) if strata==33 // Stratum 3 name 
