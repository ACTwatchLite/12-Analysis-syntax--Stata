...
********************************************************************************/
/*
This .do file calculates antimalarial and diagnostic product prprice variables per unit and per AETD in local currency and USD, using CPI and exchange rate macros set earlier (2.1). These variables are used in price tables.
*/
...
******************************************************************************* 
**# 2.9.1  DOCUMENT & SET MACROS FOR PRICE
*******************************************************************************
...
*$$$ SET LOCAL MACROS BASED ON SURVEY SPECIFICS RECORDED IN THE DOCUMENTATION ABOVE
  ...
  * EXAMPLE:
	local	cpiFIRST 	= 1.1786345  //  inflation rate since first year of time series for analysis = 2023 (1.181 * 1.001 * 0.997)
...
*###To calculate the percentage mark-up, first calculate the wholesale purchase price per unit (tablet, suppository, sachet, ml, mg (powder injection)).
				...
				*replace supplier_price_all=supplier_price_oth if supplier_price_all==. \\variable removed from form
...
* Wholesale price per AETD, data collection period, local currency units (DC LCU)
				...
				*replace ws_price_all=ws_price_oth if ws_price_al==. \\variable removed from form
...
gen unitsuppPrice=supplier_price / supplier_amt
