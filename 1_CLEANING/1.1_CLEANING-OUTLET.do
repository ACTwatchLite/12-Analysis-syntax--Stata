...
********************************************************************************/
/*This syntax imports, prepares, & cleans data captured for each outlet visited during the study, as well as screening & provider interview data for outlets where this was conducted 
*/
...
*Please inital all comments/ responses & make note of changes.  

  
 /*
 	THERE ARE MANUAL STEPS WITHIN *EACH* DO FILE FLAGGED WITH "*$$$"
	REVIEW THESE & MANAGE/CLEAN DATA ACCORDINGLY. 
	ONCE ALL *$$$ ARE ADDRESSED IN ALL .DO FILES, THIS MASTERFILE CAN BE USED TO RUN DATA PREP, CLEANING, & MANAGEMENT THEN PRODUCE OUTPUT TABLES.
	
STEPS: 
1.1.1 - *$$$ DATA PREPARATION*
1.1.2 - *$$$ DATA CLEANING**

*manual input is only required for the raw .csv data file name in the preparation syntax & for checking duplicates/ known errors
**manual input is required for checking & cleaning each variable in the dataset. The syntax below can be used to structure this cleaning process, but additional code, checks, outputs, etc. should be well documented as changes are made. 

THE RESULTS OF THIS SYNTAX = 
A cleaned dataset for each outlet with outlet info, screening, & provider interview data
...
*A subset of variables from this dataset will be merged to audit & supplier data so this syntax should be run first 
...
* import data from primary .csv file	
	/* EXAMPLE:
	insheet using "${datadir}/raw/AwL-TST-2040-final.csv", names clear
	*/
...
*save "${datadir}/1_CLEANING/AwL_${country}_${year}-outlet.dta", replace
...
*run .do file for variable labels for this dataset. 
	...
	*REMOVE AFTER WHO TESTING: adding vars for new label .do that didnt exist in nigeria form	
		gen p25=.
		gen sa14_1=.
		gen sa14_2=.
		gen sa14_3=.
	...
	do "${analydir}/1_CLEANING/variable-lables/varlbl_syntax_outlet.do"
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	*STRINGS 
		...
		*trim & convert string to uppercase **USING STRTU PROGRAM**
			strtu
...
*1.1.1.4: CHECK & CORRECT DUPLICATES
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
	...
	*$$$ If duplicates exist, investigate & resolve below	
...
*1.1.1.5: CHECK LINKS 
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
*each outlet may have up to 4 linked datasets: antimalarial audits, rdt audits, antimalarial suppliers, & rdt suppliers. Check that each outlet has the keys to link to the datasets relevant to that outlet (e.g. if the outlet has rdts, there should be an rdtaudit link)	
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
	/*All variables that are not required for cleaning & analysis should be
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
save "${datadir}/1_CLEANING/AwL_${country}_${year}-outlet_prepared.dta", replace
	...
	*notes list
	...
	/*
	### Note each record in the prepared dataset represents one form submitted & 
	each record does not represent a outlet visited i.e. some outlets have more 
	than one form (supplementary forms)
	
	the variable key & outletid_orig uniquely identify forms 
	the variable outletid uniquely identifies outlets 
	
	both are needed for merging data to the repeat group datasets (am & rdt 
	audits & suppliers datasets)
...
*******************************************************************************/
/*
In this step, each variable in the dataset is reviewed, checked, & cleaned. 
...
For outlet data, it is recommneded that the following are completed ***during fieldwork*** as part of quality control activities 
	- checking metadata, reading & responding to comments (SEE METADATA SECTION BELOW)
...
* Open prepared dataset if not already in use
	*clear 	
	*use "${datadir}/1_CLEANING/AwL_${country}_${year}-outlet_prepared.dta", replace
	count //

	
	
	
*CHECK & CLEAN EACH VARIABLE	
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	
	*$$$ REVIEW & CLEAN EACH VARIABLE IN THE OUTLET DATASET BELOW 
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  /*Review interview comments. Any interview comments that require cleaning of
    outlet-level variables should be flagged & cleaned in the respective
    cleaning step. */
...
**************** DURATION OF INTERVIEW
	...
	* Check form with shorted & longest duration 
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~			
/* The following syntax is organized by section of the toolkit quantitative questionnaire. Each variable should be checked & cleaned. 
...
*CHECK GPS VALUES FOR REPEAT VISITS TO THE SAME OUTLET
  /*Review observations with identical GPS values. Drop outlets that are
	...
	*$$$ review & remove any duplicate records. Note GPS points may overlap when outlets are very near e.g. within the same marketplace  
...
**************** OUTLET TYPE	
/*the questionnaire captures retail & wholesale outlet type in seperate variables. 
	...
	*check outlet type & recode outlet type other specify responses to predefined values
		ta c7, m 
			/* INSERT RESULTS:
			*/
...
**************** OUTLET NAME 
	*$$$ outlet names may be cleaned for for obvious errors. 
		*/ Check that duplicated names are reasonable (e.g. chain outlets), check outlet names are consistant with type (e.g. if there is 'pharmacy' in the name the outlet type should likely also be pharmacy). If there are suspiciously similar outlet names of the same type in the same cluster then investigate & potentially drop duplicates. This variable is not used in the analysis so manual cleaning is not required unless specified*/ 
...
********************************************************************************
**# CENSUS & SCREENING MODULE
	*1.1.2.4: SECTION 2: SCREENING & ELIGIBILITY                               
********************************************************************************  
/*Checks variables related to screening, eligibility, & consent then recodes 
...
*INCLUSION CRITERIA:
	*Do you have any antimalarial medicine in stock today?
			...
			ta am_stockcurrent, m //this is a calculated variable from the form & should match s3
				/* INSERT RESULTS:
				*/
	...
	*Have you stocked AMs in the last 3 months?
			...
			ta am_stockpast, m //this is a calculated variable from the form & should match s4
				/* INSERT RESULTS:
				*/
	...
	*Are RDTs available here today?
			...
			ta rdt_stock //this is a calculated variable from the form & should match s5b
				/* INSERT RESULTS:
				*/
	...
	*Is malaria microscopy screening available here today?
			...
			ta micro_stockcurrent //this is a calculated variable from the form & should match s5a
				/* INSERT RESULTS:
				*/
	...
	*any testing calculate variable 
			...
			ta testing_stock //this is a calculated variable & should = outlets with RDT and/or microscopy 
				/* INSERT RESULTS:
				*/
...
**************** ELIGIBILITY & CONSENT
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	*outlets are eligible if (1) they were screened & 
	...
	*check calculate variables summarizing eligibility for study components 	
			...
			ta prov_int, m //outlet is eligible, consented, & outlet type are applicable for provider interview 
			ta do_audit, m //outlet is eligible, consented, & outlet type are applicable for audit
...
*It will combine information from Section 2 on screening, eligibility, & consent with the progress & completeness of the rest of the interview & audits
*/ 

*********** CREATE NEW STATUS VARIABLE & RECODE
	gen c9_new = c9 
...
**$$$ the codes used in the odk form need to be recoded to match the categories used for the analysis (the label definitions above. The following syntax recodes these & fixes any miscategorizations based on other variables e.g. checkpoints & audit compoletness 
...
*********** INTERVIEW INTERUPTED - CHECKPOINTS
	...
	interview was interrupted*/ 
		...
		gen outletint_interupted=0 
			cap recode outletint_interupted 0=1 if checkpoint1==. | checkpoint2==. | ///
		...
		replace c9_new=2 if outletint_interupted==1
 ...
 *Incomplete antimalarial audit 
	 ta c9_new amaudit_complete, m
		...
		*$$$ recode reasons for incomplete audit & double check that FinIntStat is recoded to reflect this.
...
/**$$$ Note before the am & rdt audit, outlets are asked again if they 
*have ams & rdts. This is checked for consistancy with screening in section 3 
...
If your survey uses a different eligibility critiera, the syntax "prov_int=1 & $inf!=1" should be updated

Similarly, if sections are added or removed, the numbering of checkpoints in your survey may be differnt. Check & edit these as required.
*/		
...
*Does this outlet have the correct license/ registration to sell medicines here?
	...
	2	YES - the repondent REPORTS & you have OBSERVED the licence
	...
	These values & labels will vary based on country context. Cross check & edit syntax accordingly
	*/
...
* Do you or anyone in this outlet/ business have any of the following health-related qualifications? Check missing values
	ta p8 if (prov_int==1 & inf!=1 & checkpoint1==1) , m
		/* INSERT RESULTS:
		*/
...
*Has any staff working at this outlet/ business received malaria-related training in the last 2 years? Checking missing values?
	ta char9 if (prov_int==1 & inf!=1 & checkpoint1==1) , m
	/* INSERT RESULTS:
		*/
	...
	*clean other specified 
	list char9_other if char9_other!="" 
...
**************** 1.1.2.5.3:	Section 3: Quality control & compliance                                
...
* Please name one first-line drug <b>recommended by the government</b> to treat uncomplicated malaria. (i.e. what is the most effective drug to treat uncomplicated malaria)? Check missing values
	list p16_name_oth if p16_name_oth!=""
	
		*$$$ clean & recode as needed
	
	bysort p16_form: ta p16_name if (prov_int==1 & inf!=1 & checkpoint3==1)  
		/* INSERT RESULTS:
		*/
...
*Have you ever seen or heard of a malaria rapid diagnostic test (RDT)? Missing values
	bysort c7: ta p21 if (prov_int==1 & inf!=1 & checkpoint3==1)  , m
		/* INSERT RESULTS:
		*/
...
*While working at this outlet/ business, have you ever tested a client for malaria using an RDT? Checking missing values
	bysort c7: ta p22 if (prov_int==1 & inf!=1 & checkpoint3==1) & p21==1  , m
		/* INSERT RESULTS:
		*/
...
*Is a prescription or positive malaria test result required to purchase an antimalarial at this outlet?
	bysort c7: ta p25 if (prov_int==1 & inf!=1 & checkpoint3==1) , m
		/* INSERT RESULTS:
		*/
...
*Would you recommend that a patient/client take an antimalarial drug if the rapid diagnostic test is negative for malaria?
	bysort c7: ta p23 if (prov_int==1 & inf!=1 & checkpoint3==1) & p21==1  , m
		/* INSERT RESULTS:
		*/
...
Note these responses may have been edited for this study. Cross check & update accordingly.
*/

	list p24_other if p24_other!=""
	
		*$$$ clean & recode as needed
		
	ta p24 if (prov_int==1 & inf!=1 & checkpoint3==1) , m
		/* INSERT RESULTS:
		*/
...
*In the past 30 days, did this outlet/ business have...
	ta dig0 if (prov_int==1 & inf!=1 & checkpoint4==1), m			//water	
	ta dig1 if (prov_int==1 & inf!=1 & checkpoint4==1), m				//electricity
	ta dig2 if (prov_int==1 & inf!=1 & checkpoint4==1), m				//any phone
	ta dig2c if (prov_int==1 & inf!=1 & checkpoint4==1) & dig2!=0, m 	//mobile connection
	ta dig3 if (prov_int==1 & inf!=1 & checkpoint4==1), m				//wifi
	ta dig5 if (prov_int==1 & inf!=1 & checkpoint4==1), m				//tablet or computer
		/* INSERT RESULTS:
		*/
...
*Select which of the following applications/ services you have used on this phone in the last 30 days:
	/* 	3	Mobile5 money
		...
		ta dig2d if (prov_int==1 & inf!=1 & checkpoint4==1) & dig2!=0, m 	//apps used 
		/* INSERT RESULTS:
		*/
...
*For each of the following business activities, identify if you currently use digital technology, would like to digitize, or are not interested in digitizing the activity. 
		ta dig7_1 if (prov_int==1 & inf!=1 & checkpoint4==1), m	//Gestion des ventes au détail aux clients
		ta dig7_2 if (prov_int==1 & inf!=1 & checkpoint4==1), m	//Gestion des stocks 
		ta dig7_3 if (prov_int==1 & inf!=1 & checkpoint4==1), m	//Passer des commandes auprès des fourisseurs
		ta dig7_4 if (prov_int==1 & inf!=1 & checkpoint4==1), m	//Payer les fournisseurs
		ta dig7_5 if (prov_int==1 & inf!=1 & checkpoint4==1), m	// HR
		...
		*other activities 
		ta dig7b if (prov_int==1 & inf!=1 & checkpoint4==1), m
		
		list dig7bi if dig7bi!=""
		
			*$$$ review & recode others as needed
...
* Do you report malaria case data each month?
	ta data1 c7 if (prov_int==1 & inf!=1 & checkpoint5==1), m
		/* INSERT RESULTS:
		*/
			
		**$$$ sense check, clean as needed 			
		  
* What forms & tools do you use to <b>report</b> malaria-related data to higher levels of the health system?
		*Other forms & tools used to report data:
		list data2b if data2b!=""
		...
		*Check form type variables 
		...
		Form types should be updated based on country context. Cross check values & edit syntax accordingly
		*/ 
		
			/* EXAMPLE:
			ta data2 c7 if (prov_int==1 & inf!=1 & checkpoint5==1) & data1==1, m  
			ta data2_1 c7 if (prov_int==1 & inf!=1 & checkpoint5==1) & data1==1, m  
			ta data2_2 c7 if (prov_int==1 & inf!=1 & checkpoint5==1) & data1==1, m  
			ta data2_3 c7 if (prov_int==1 & inf!=1 & checkpoint5==1) & data1==1, m  
			ta data2_96 c7 if (prov_int==1 & inf!=1 & checkpoint5==1) & data1==1, m  
			ta data2_98 c7 if (prov_int==1 & inf!=1 & checkpoint5==1) & data1==1, m 
			*/
...
***Check consistency & missing values for following quesions
...
*Which of the following types of clients or businesses do you sell antimalarials or mRDTs to?
	*	ta p32 if prov_int==1 & inf!=1 & checkpoint6==1 & retws==1, m	
		...
		*Other customer types
		list p32b if p32_6==1
		
			*$$$ review & recode other customer types as needed
...
*What br& of antimalarial do you sell to Individual clients or use most often at this facilty/outlet?
	list sa11
...
*Where do you import antimalarials from (include company names & countries where possible). 
	list ws1b if ws1==1
...
*Note most of the antimalarial data is captured in the "repeat group" of the form & stored in a separate dataset (antimalarial audit dataset). Those variables will be cleaned separate (step 1.2)   
...
*AUDIT COMPLETENESS 
	...
	ta amaudit_incomplete if amaudit_complete!=0, m 
		...
		*$$$ Check missing values & recode reasons for incomplete audit
...
This data is exported in to the supplier dataset & cleaned in step 1.2***/			


********************************************************************************
**# 1.1.2.7: BLOOD TESTING AUDIT MODULE 

*Note most of the RDT data is captured in the "repeat group" of the form & stored in a seperate dataset (rdt audit dataset). Those variables will be cleaned seperatly (step 1.3)   
...
/***The rest of this section of the questionnaire is contained within a repeat group. 
This data is exported in to the supplier dataset & cleaned in step 1.3***/	
...
*Note most of the supplier data is captured in the "repeat group" of the form & stored in a seperate dataset (supplier dataset). Those variables will be cleaned seperatly (step 1.4)                        
********************************************************************************  
*Thinking again about your main suppliers for malaria commodities, are you able to share specific details about your main suppliers such as name & location?  
...
/***The rest of this section of the questionnaire is contained within a repeat group. 
This data is exported in to the supplier dataset & cleaned in step 1.4***/
	




*** NOTE THE INTERVIEW RESULTS MODULE IS CLEANED IN ELIGIBILITY & CONSENT ***                    
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~	
save "${datadir}/1_CLEANING/AwL_${country}_${year}-outlet_clean.dta", replace
