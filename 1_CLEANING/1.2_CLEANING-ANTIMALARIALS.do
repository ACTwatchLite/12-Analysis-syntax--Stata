...
* import data from primary .csv file	
	/* EXAMPLE:
	insheet using "${datadir}/raw/AwL-${country}-${year}-amAudit.csv" 
	 */
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	*REMOVE AFTER WHO TESTING: adding vars for new label .do that didnt exist in nigeria form	
		rename excip hassalt
		rename excip_salt salt
		rename excip_salt_other salt_other
		gen salton=.
		gen salton_other=.
		rename nafdac_code reg_code
		
	
	
	
	do "${analydir}/1_CLEANING/variable-lables/varlbl_syntax_amaudit.do"
...
*Use key from all forms to merge relevant cleaning variables
	  ...
	  merge m:1 key using ///
		"${datadir}/1_CLEANING/AwL_${country}_${year}-outlet_clean.dta", ///
...
*MERGE AM DATABASE
cap drop _merge
	merge m:1 amcode using "${analydir}/lists/antimalarial_masterlist.dta"
/*
	*$$$ add merge results:
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
save "${datadir}/1_CLEANING/AwL_${country}_${year}-amaudit_prepared.dta", replace
...
* Open prepared dataset if not already in use
	clear 	
	use "${datadir}/1_CLEANING/AwL_${country}_${year}-amaudit_prepared.dta", clear
...
*/
	...
	*run basic string cleaning 
	...
	replace brand=regexr(brand,"9998","")
...
**review photo...
*e.g. size 24 g in dataset but photo review shows it should be 60 ml
...
*/  
		...
		*recode (-99 -98 -98 = .) //@PB too few variables specified
...
*CREATE VARS THAT CAN BE MATCHED BETWEEN DATA COLLECTION DATABASE AND MAIN PRODUCT DATABASE
       ...
       save "${datadir}/1_CLEANING/AwL_${country}_${year}-amaudit_clean.dta", replace
...
duplicates drop amcode temp_product_code , force
			...
			duplicates drop temp_product_code , force //there should not be duplicates. investigate
				
		save "${analydir}/lists/product_codes.dta", replace

		use "${datadir}/1_CLEANING/AwL_${country}_${year}-amaudit_clean.dta", replace
		
		cap drop _merge
		
	
		merge m:1 temp_product_code using  "${analydir}/lists/product_codes.dta"
...
*MERGE AM DATABASE
	...
	use "${datadir}/1_CLEANING/AwL_${country}_${year}-amaudit_clean.dta", clear
...
* THIS WILL NEED TO BE UPDATED TO REFLECT RECENT CHANGES. CHECK THE WHO/Global Fund WEBSITES FOR THE MOST RECENT LIST
		...
		*Note this list contains some pre-qualified antimalarials that are not ACTs (e.g. SP) here we only want antimalarials so sytax only recodes for set generic names below.
		recode qaact 0=1 if qaact_master==1 & (gname==40 | gname==42 | gname==47 | gname==49 | gname==55 ///
...
*Generate the NATAPP variable.
	...
	*recode for searched products 
		...
		*recode natapp 0=1 if natapp_master==1
		
		recode natapp 0=1 if natapp_master=="IDENTIFY WHICH PRODUCTS ARE APPROVED IN YOUR COUNTRY HERE; ADD ROWS FOR MISSING PRODUCTS THAT ARE APPROVED"
