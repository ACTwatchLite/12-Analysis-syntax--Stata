...
**** USER NUMBERS: 1 = [Sara] ; 2 = [Jim]; 3 = [Jo]
	*global user 1 //Paul
	global user 2 //Katelyn
	*global user 3 //Jo
	
  if $user ==1 {
		//change the stata directory to set the main analysis directory for user1 here: 	
		cd "/Users/paulbouanchaud/Library/CloudStorage/OneDrive-PopulationServicesInternational/Documents - ACTwatch Lite/2. Technical/0. Toolkit/ACTwatch Lite Toolkit v4/12 Analysis syntax"
		
		
		//create a global macro to identify the main project directory location here 
		global projdir "/Users/paulbouanchaud/Library/CloudStorage/OneDrive-PopulationServicesInternational/Documents - ACTwatch Lite/2. Technical/0. Toolkit/ACTwatch Lite Toolkit v4"
		
		display "***** User set to user 1 *****"
				}
				
		
	else if $user ==2 {
			//change the stata directory to set the main analysis directory for user1 here: 	
		cd "C:\Users\kwoolheater\Population Services International\ACTwatch Lite - Documents\2. Technical\0. Toolkit/ACTwatch Lite Toolkit v4/12 Analysis syntax"
		
		
		//create a global macro to identify the main project directory location here 
		global projdir 	"C:\Users\kwoolheater\Population Services International\ACTwatch Lite - Documents\2. Technical/0. Toolkit/ACTwatch Lite Toolkit v4"
		
		
		display "***** User set to user 2 *****"
				}
			
							
	else if $user ==3 {
		//change the stata directory to set the main analysis directory for user1 here: 	
		cd "C:\Users\JO\Population Services International\ACTwatch Lite - Documents\ACTwatch Lite Toolkit\12 Analysis syntax"
...
*******************************************************************************
* These .do files merge data in to one analysis dataset, weight and data, and add variables needed for results generation 
*	  
...
**# STEP 3.1 set local definitions for antimalarials and rdts
*******************************************************************************
...
Review these .do files and modify as needed (e.g. outlet types, antimalarial categories, etc): 
	3.1a_LOCAL_DEFINITIONS antimalarials.do
	3.1b_LOCAL_DEFINITIONS diagnostics.do

There is no need to run them here. The appropriate definitions file will be called in each tables .do file in step 3.2
...
**# STEP 3.2 modify and run flow diagram syntax
*******************************************************************************
	do "${resultsdir}/3.2_FLOW-DIAGRAM.do"
	

**# STEP 3.2 modify and run tables syntax 
*******************************************************************************
	do "${resultsdir}/3.3_MODIFY_AND_RUN_TABLE_SYNTAX.do"
	
	*3.X 
		//Table .do files are numbered to match the corresponding indicator in the indicator table 
		*Use the .do file above or syntax below to run each relevant .do file for indicators assessed in this study
		*comment out .do files for indicators that have been removed.
		
				

		/*	COPY OF STUNTAX FROM 3.3**** 
