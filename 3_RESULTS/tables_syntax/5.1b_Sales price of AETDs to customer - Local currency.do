...
**ENSURE THAT ALL OUTLETS INCLUDED (with at least 1 product available) ARE CODED AS EITHER 1 OR 0 FOR THIS ANALYSIS (I.E. NO MISSING VALUES)			
...
local tabname T_i	
	...
	*file name and sheet
	putexcel set "${tables}/5.1b_Sales price of tablet AETDs local currency_$tabver", sheet("`tabname'") modify
	...
	*labels	
	...
	count if anyAM==1 & ppriceperaetd==. // COUNTS THE N PRODUCTS MISSING PRICE DATA
	return list
	local footnote2 =r(N)
	...
	*combination. Products are listed in the excel on rows, outlets in columns. 
		...
		_pctile ppriceperaetd [pw=wt_allOutlet] if `y'==1 & `x'==1 & ppriceperaetd!=., p(25, 50, 75)
			...
			count if `y'==1 & `x'==1 & countprod==1 & ppriceperaetd!=.
...
*table label
	...
	***store the count to include in footnote
	...
	count if anyAM==1 & ppriceperaetd==. & Ru1==1
	return list
	local footnote2 =r(N)
	...
	count if anyAM==1 & ppriceperaetd==. & Ru2==1
	return list
	local footnote4 =r(N)

	
	
	putexcel C1=  ("Footnote: Prices are per AETD of tablet formulations only. N rural outlets that met screening criteria for a full interview but did not complete the interview (were not interviewed or completed a partial interview) = `footnote1'; N antimalarial products audited in rural outlets but missing price information = `footnote2'")
	putexcel D1=  ("N urban outlets that met screening criteria for a full interview but did not complete the interview (were not interviewed or completed a partial interview) = `footnote3'; N antimalarial products audited in urban outlets but missing price information = `footnote4'")
	...
	**tables loop and putexcel output	
		...
		_pctile ppriceperaetd [pw=wt_allOutlet] if `y'==1 & `x'==1 & `z'==1 & ppriceperaetd!=., p(25, 50, 75)
...
local tabname T_iii_`s'		
	...
	***store the count to include in footnote?
	...
	count if anyAM==1 & ppriceperaetd==. 
	return list
	local footnote2 =r(N)
	...
	**tables loop and putexcel output	
		...
		_pctile ppriceperaetd [pw=wt_allOutlet] if `y'==1 & `x'==1  & `s'==1 & ppriceperaetd!=., p(25, 50, 75)
			...
			count if `y'==1 & `x'==1 & `s'==1 & countprod==1 & ppriceperaetd!=.
...
********************************************************************************
********************************************************************************
*VI. Rural/Urban & State level results
********************************************************************************
********************************************************************************
...
local tabname T_iv_`s'		
	...
	* Rural	
	...
	count if anyAM==1 & ppriceperaetd==. & `s'==1 & Ru1==1
	return list
	local footnote2 =r(N)
	...
	* Urban	
	...
	count if anyAM==1 & ppriceperaetd==. & `s'==1 & Ru2==1
	return list
	local footnote4 =r(N)

	
	
	putexcel C1= ("`s' footnote: Prices are per AETD of tablet formulations only. N rural outlets that met screening criteria for a full interview but did not complete the interview (were not interviewed or completed a partial interview) = `footnote1'; N antimalarial products auditedin rural outlets but missing price information = `footnote2'")
	putexcel D1= ("N urban outlets that met screening criteria for a full interview but did not complete the interview (were not interviewed or completed a partial interview) = `footnote3'; N antimalarial products audited in urban outelts but missing price information = `footnote4'")
	...
	**tables loop and putexcel output	
		...
		_pctile ppriceperaetd [pw=wt_allOutlet] if `y'==1 & `x'==1  & `z'==1  & `s'==1 & ppriceperaetd!=., p(25, 50, 75)
			...
			count if `y'==1 & `x'==1 & countprod==1  & `s'==1 & ppriceperaetd!=.
