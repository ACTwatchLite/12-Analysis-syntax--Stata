*	1		Availability
*	1.3 Availability of malaria blood testing among antimalarial stocking outlets
...
*table label
	...
	*file name and sheet
	putexcel set "${tables}/2.4_Availability of tests in all antimalarial-stocking outlets_$tabver", sheet("`tabname'") modify
	...
	**tables loop and putexcel output	
	...
	foreach x of varlist `testvars' { 					// local macro indicating variable list for diagnostics - from local_definitions.doh
		local row=`row'+1
		local col=2	
		...
		local varlabely :  var label `y'
			...
			estpost svy, subpop(if anyAM==1 & `y'==1): tab `x', ci per nototal
...
********************************************************************************
********************************************************************************
*II. National results by rural-urban
********************************************************************************
********************************************************************************
...
*table label
	...
	***store the count to include in footnote?
	...
	putexcel C1= ("`s' Footnote - Number of rural outlets found stocking one or more antimalarials: Private not for profit=`footnote1'; private not for profit=`footnote2'; pharmacy=`footnote3'; PPMV=`footnote4'; informal=`footnote5'; labs = `footnote6'; wholesalers= `footnote7'. Outlets that had at least 1 AM in stock but did not complete the interview (were not interviewed or completed a partial interview) = `footnote8' ")
	putexcel D1= ("N urban AM-stocking outlets: Private not for profit=`footnote9'; private not for profit=`footnote10'; pharmacy=`footnote11'; PPMV=`footnote12'; informal=`footnote13'; labs = `footnote14'; wholesalers= `footnote15'. Outlets that had at least 1 AM in stock but did not complete the interview (were not interviewed or completed a partial interview) = `footnote16' ")
	...
	**tables loop and putexcel output	
	...
	foreach x of varlist `testvars' {
		...
		local varlabelz :  var label `z'
			...
			estpost svy, subpop(if anyAM==1 & `y'==1 & `z'==1): tab `x', ci per nototal
...
*table label
	...
	*labels	
	...
	putexcel C1= ("`s' Footnote - Number of outlets stocking one or more antimalarials: Private not for profit=`footnote1'; private not for profit=`footnote2'; pharmacy=`footnote3'; PPMV=`footnote4'; informal=`footnote5'; labs = `footnote6'; wholesalers= `footnote7'. Outlets that had at least 1 AM in stock but did not complete the interview (were not interviewed or completed a partial interview) = `footnote8' ")
	...
	**tables loop and putexcel output	
	...
	foreach x of varlist `testvars' {
		...
		local varlabelx :  var label `x'
			...
			estpost svy, subpop(if anyAM==1 & `y'==1 & `s'==1): tab `x', ci per nototal
...
*table label
	...
	*RURAL
*Footnote: Number of screened outltes with at least one antimalarial stocked, for each outlet type
...
*Footnote: Number of screened outltes with at least one antimalarial stocked, for each outlet type
	...
	*labels	
	...
	putexcel C1= ("`s' Footnote - Number of rural outlets stocking one or more antimalarials: Private not for profit=`footnote1'; private not for profit=`footnote2'; pharmacy=`footnote3'; PPMV=`footnote4'; informal=`footnote5'; labs = `footnote6'; wholesalers= `footnote7'. Outlets that had at least 1 AM in stock but did not complete the interview (were not interviewed or completed a partial interview) = `footnote8' ")
	putexcel D1= ("N urban AM-stocking outlets: Private not for profit=`footnote9'; private not for profit=`footnote10'; pharmacy=`footnote11'; PPMV=`footnote12'; informal=`footnote13'; labs = `footnote14'; wholesalers= `footnote15'. Outlets that had at least 1 AM in stock but did not complete the interview (were not interviewed or completed a partial interview) = `footnote16' ")
		...
		local varlabelz :  var label `z'
			...
			local varlabely :  var label `y'
				...
				qui: count if `x'==1 & `y'==1 & `z'==1 & `s'==1 & anyAM==1
					if r(N)==0 {
					qui: count if `y'==1 & `z'==1 & `s'==1 & anyAM==1
						matrix ns=r(N)
			...
			estpost svy, subpop(if anyAM==1 & `y'==1 & `z'==1 & `s'==1): tab `x', ci per nototal
