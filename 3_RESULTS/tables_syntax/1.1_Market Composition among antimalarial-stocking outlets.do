...
*EXAMPLE:
use "${mngmtdata}/${country}_${year}_outlet_data.dta", clear
...
*table label
	...
	*labels	
	...
	putexcel C1= ("Footnote: Rural outlets that met screening criteria for a full interview but did not complete the interview with any AM in stock = `footnote1' ")
	putexcel D1= ("Urban outlets that met screening criteria for a full interview but did not complete the interview with any AM in stock = `footnote2' ")
	...
	**tables loop and putexcel output	
		...
		recode `x'(.=0) if st_anyAM==1
			...
			estpost svy, subpop(if st_anyAM==1 & booster==0 & `z'==1): tab `x', ci per nototal
...
local tabname T_iii_`s'		
	...
	*labels	
	...
	putexcel C1= (" Footnote: `s' outlets that met screening criteria for a full interview but did not complete the interview with any AM in stock = `footnote1' ")
